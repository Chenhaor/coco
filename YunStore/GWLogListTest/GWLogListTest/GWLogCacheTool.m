...
/*
#import <GWCacheFramework/GWCacheFramework.h>
#import "GWLogModelDAO.h"

static GWLogCacheTool *_shareInstance;

@interface GWLogCacheTool ()

@property (nonatomic, strong) GWDBManager *dbManager;
@property (nonatomic, strong) GWLogModelDAO *logDAO;
@end
...
{
    {
        ...
        dispatch_once(&onceToken, ^{
            _shareInstance = [[GWLogCacheTool alloc] init];
...
- (void)setup
{
    _dbManager = [[GWDBManager alloc] initDBWithName:@"log.sqlite"];
    
    NSString *createTableSQL = @"create table if not exists t_log (id integer primary key autoincrement, logCreatTime integer, softVersion text, logType integer, logMsg text, fileMsg text);";
    [_dbManager createTableWithSql:createTableSQL];
    _logDAO = [[GWLogModelDAO alloc] initWithDBQueue:_dbManager.queue];
}
