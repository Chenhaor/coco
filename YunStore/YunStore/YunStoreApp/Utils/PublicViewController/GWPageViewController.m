...
#import "GWPageViewController.h"

@interface GWPageViewController ()
<
...
@implementation GWPageViewController
...
- (void)setPageContentFrame:(CGRect)pageContentFrame
{
    ...
    self.view.frame = pageContentFrame;
}
...
- (void)setContentControllers:(NSArray<GWPageContentViewControllerDelegate> *)contentControllers
{
    if (_contentControllers != contentControllers)
    {
        ...
        else
        {
            for (NSInteger i = 0; i < contentControllers.count; i++) {
                ...
                UIViewController<GWPageContentViewControllerDelegate> *content = [contentControllers objectAtIndex:i];
                
                content.pageContentFrame = self.pageContentFrame;
            ...
            [self.pageController setViewControllers:@[show]
                                          direction:UIPageViewControllerNavigationDirectionForward
                                           animated:NO
                                         completion:nil];
//            show.view.frame = self.pageController.view.bounds;
            [self pageViewController:self.pageController
  viewControllerBeforeViewController:show];
            [self pageViewController:self.pageController
   viewControllerAfterViewController:show];
